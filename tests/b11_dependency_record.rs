...
//!
//! The workspace manifests, which are written by hand and name nine dependencies: `exr`, `gif`, `image`, `png`, `rayon`
...
fn b11_the_dependency_record_describes_the_build_it_claims_to() {
    ...
    // Save dialogs, `exr` with D-62, `image` and `gif` with D-72, and this line is how a dependency added quietly stops being
    // quiet.
    ...
    report.check(
        ...
        "exr, gif, image, png, rayon, serde_json, tauri, tauri-build, tauri-plugin-dialog",
        joined(&direct),
    );
