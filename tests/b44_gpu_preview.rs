...
//! `verification/B-44_gpu_timing_table.md`.
...
fn b44_gpu_preview() {
    ...
    let g = gpu.draw(&hard).unwrap_or_else(|d| panic!("the hard case on the GPU: {}", d.message));
    ...
    for quality in [PreviewQuality::Full, PreviewQuality::Draft] {
        for mode in [BlendMode::Normal, BlendMode::Multiply, BlendMode::Screen, BlendMode::Add] {
            ...
            let g = gpu.draw(&p).unwrap_or_else(|d| panic!("{mode:?} on the GPU: {}", d.message));
            t.compare(
    ...
    let (a, b) = (gpu.draw(&p).expect("draw once"), gpu.draw(&p).expect("draw again"));
    t.row(
    ...
    // The card's store under pressure: a budget of one drawing, so nearly every drawing is sent
    // and dropped again, across the reference shot. A drawing the store confused with another
    // would show here as a wrong picture.
    let budget = gpu.budget;
    gpu.budget = 1;
    let mut worst = (0u8, 0usize);
    for frame in (0..240).step_by(5) {
        let p = plan(reference, frame, PreviewQuality::Draft);
        let g = gpu.draw(&p).expect("draw with the small budget");
        worst = worst.max(distance(&cpu(&p), &g));
    }
    gpu.budget = budget;
    t.row(
        "the reference shot, every fifth frame at Draft, the card keeping one drawing at a time",
        "at most 1 level",
...
fn b44_gpu_timing() {
    ...
    let mut s = format!(
        "# B-44: frame times, CPU and GPU\n\n\
         ...
         drawings already on the card. Medians over every frame of the shot, in ms.\n\n\
    ...
    fs::write(repo("verification/B-44_gpu_timing_table.md"), s).expect("write the timing table");
}
