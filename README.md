...
| `app/` | The `anime_compositor_app` crate: the Tauri shell, the page in `app/ui/index.html`, and the window's own tests. |
...
| `tools/` | Small generators and checks for committed artifacts. `gen_dependencies.py` writes `docs/DEPENDENCIES.md` from `cargo metadata` and `Cargo.lock`; `capture_window.ps1` takes the photographs; `audit_references.py` reads every document and reports a named file that does not exist or a score quoted differently from the artifact that states it. |
