...
**Status: G1-core implementation. B-02 through B-05c, B-09 and the headless half of B-08 are complete; export to an image sequence is next.** G0 passed on 2026-09-04. Planning is complete, the reference shot is drawn, the G0 spikes are measured and recorded, the colour and compositing core is implemented and passing its fixtures, PNG sequences import with gap, Unicode and format diagnostics, the reference shot evaluates to a drawing number per layer per frame at exact rational time, the project model takes every edit through a command interface with exact undo and redo, a tiled multithreaded renderer turns a layer's anchor, position, scale, rotation and opacity into pixels with output that does not depend on tile size or thread count, trace mode writes every intermediate layer buffer of a frame as a tagged PNG so a wrong picture can be diagnosed by looking rather than by reading code, a project can be saved, closed and reopened without losing anything, including the parts of a file this build does not understand, a layer composites in any of document 21's four blend modes, a warning that repeats on every frame of a shot is logged a few times and then summarised with its count and frame ranges rather than twenty times over, and - as of B-08a - a saved project file and a frame number are enough to produce that frame: `verification/B-08a_frames/` holds four frames of the reference shot rendered from `verification/B-08a_project.json`, including one of the frames where a drawing is deliberately missing and nothing is substituted for it. What does not exist yet is a window to show them in, and a way to write them to a folder.
