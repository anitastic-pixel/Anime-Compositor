...
**Status: G1-core implementation. B-02, B-03 and B-04 are complete; B-05 is next.** G0 passed on 2026-09-04. Planning is complete, the reference shot is drawn, the G0 spikes are measured and recorded, the colour and compositing core is implemented and passing its fixtures, PNG sequences import with gap, Unicode and format diagnostics, and the reference shot now evaluates to a drawing number per layer per frame at exact rational time.
