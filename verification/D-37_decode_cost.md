...
**What 57 cels costs is 1.89 GB.** This line originally said 473 MB, and 473 MB is the wrong number for the thing being decided. It prices a cel at 1920 x 1080 x 4 bytes, which is what the PNG decoder writes: 8-bit RGBA. It is not what a cache holds. `src/cache.rs` holds a decoded cel as linear premultiplied f32 RGBA and bounds itself on exactly those samples, which is sixteen bytes a pixel and 33,177,600 bytes a cel, four times the decoder's output. Both figures are now printed by `verification/derive_d37_reuse.py` so that neither can be quoted for the other again. That correction changes the shape of the decision rather than a digit in it: the cheap end of the curve is a few hundred megabytes for half the saving, and **the far end is 1.89 GB, which is past the 1 GiB D-40 later set as the default budget** rather than comfortably inside it. Nothing else on this page moves; the decode timings and the reuse counts were measured and counted correctly and are unchanged.
