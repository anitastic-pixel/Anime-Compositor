...
6. **Sequence layers.** Click one layer, then Shift+click two more, in the order they should play. Right-click one of them and choose Sequence layers: the second starts where the first ends, the third where the second ends, each as long as before. One Ctrl+Z puts all three back. With only one layer chosen the line is greyed out.
...
8. **Palette.** Ctrl+Shift+P and type "shy", "sequence", "duplicate", "delete the composition" or "preferences": each has a line.
