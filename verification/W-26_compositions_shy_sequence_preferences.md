...
6. **Sequence layers.** Click layer 1, then Shift+click layer 3 (all three chosen), or Ctrl+click layers one at a time. Right-click one of them and choose Sequence layers: the top chosen row stays put, the next row down starts where it ends, the next where that one ends, each as long as before. One Ctrl+Z puts them all back. With only one layer chosen the line is greyed out.
   - Third playtest request: the menu has two lines, Sequence layers by trimmed length and by full length. Trim layer 2 from both ends. By trimmed length: layer 2's bar starts right where layer 1's bar ends, no gap. Ctrl+Z, then by full length: layer 2 sits where it would if it were untrimmed, so there is a gap before its bar the size of what was trimmed off its start, and layer 3 comes after where layer 2's untrimmed end would be. A layer with neither exposures nor drawings goes by its bar either way.
   - Second playtest fix: Shift+click had chosen the layers but only the last row lit up, so it looked as if nothing happened. Every chosen row lights up now, and Shift on a bar takes the range as Shift on a name does. Check: click layer 1, Shift+click layer 3's name or bar, and rows 1, 2 and 3 are all lit.
