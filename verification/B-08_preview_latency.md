...
This does not reopen D-33 - draft is still the faster of the two and still the right default - but it does say plainly that resolution alone will not buy real-time playback of this shot. What would is not decoding the same drawing again every time it is shown, which is document 27's cache. That is B-08b, PARKED under D-12 on the day this was measured, and this table is the first measurement in this project that argues for it from the production path rather than from a spike.

*Added 2026-09-06: that park was lifted the following day. This table is the trigger D-37 cites, the cache was built and measured in `verification/B-08b_cache_budget.md`, and `verification/B-08_window_shell.md` shows what it did to playback. The paragraph above is left as it was written, because it is the argument that unparked the work and not a description of the build as it stands.*
