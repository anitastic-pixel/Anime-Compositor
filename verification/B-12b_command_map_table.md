...
|---|---|---|---|
| document 24 names the identifiers this table walks | 94 | 94 | pass |
| and they are the same identifiers, in the same order | project.new, project.open, project.save, project.save_as, composition.create, composition.open, composition.set_settings, composition.duplicate, composition.delete, edit.undo, edit.redo, media.import, media.relink, asset.set_redistribute, project.collect, package.check, layer.create, layer.delete, layer.rename, layer.move_up, layer.move_down, layer.toggle_visibility, layer.toggle_lock, layer.set_matte, layer.set_parent, layer.set_depth, camera.set_property, layer.shift, layer.trim, layer.move, layer.duplicate, layer.split, layer.toggle_solo, layer.set_label, layer.set_blend_mode, layer.set_gain, layer.add_adjustment, layer.add_composition, layer.precompose, layer.copy, layer.paste, layer.toggle_shy, layer.add_solid, solid.set, mask.add, mask.add_remove_key, mask.set_path, mask.set, mask.delete, layer.add_shape, shape.add, shape.add_remove_key, shape.set_path, shape.set, shape.delete, path.add_point, path.remove_point, layer.add_null, property.link, timesheet.import, sheet.print, timeline.previous_frame, timeline.next_frame, timeline.play_pause, timeline.set_work_start, timeline.set_work_end, timeline.set_markers, exposure.set_span, exposure.write, exposure.toggle_key, sheet.write_action, property.set_base, keyframe.add_remove, keyframe.move, keyframe.set_interp, keyframe.set_path, keyframe.set_kind, keyframe.set_roving, property.separate, property.set_expression, effect.add, effect.delete, effect.toggle_bypass, effect.set_parameters, effect.move_up, effect.move_down, effect.move, viewer.fit, viewer.zoom_100, viewer.toggle_checkerboard, viewer.toggle_alpha, render.preview_current, export.sequence, app.command_palette | project.new, project.open, project.save, project.save_as, composition.create, composition.open, composition.set_settings, composition.duplicate, composition.delete, edit.undo, edit.redo, media.import, media.relink, asset.set_redistribute, project.collect, package.check, layer.create, layer.delete, layer.rename, layer.move_up, layer.move_down, layer.toggle_visibility, layer.toggle_lock, layer.set_matte, layer.set_parent, layer.set_depth, camera.set_property, layer.shift, layer.trim, layer.move, layer.duplicate, layer.split, layer.toggle_solo, layer.set_label, layer.set_blend_mode, layer.set_gain, layer.add_adjustment, layer.add_composition, layer.precompose, layer.copy, layer.paste, layer.toggle_shy, layer.add_solid, solid.set, mask.add, mask.add_remove_key, mask.set_path, mask.set, mask.delete, layer.add_shape, shape.add, shape.add_remove_key, shape.set_path, shape.set, shape.delete, path.add_point, path.remove_point, layer.add_null, property.link, timesheet.import, sheet.print, timeline.previous_frame, timeline.next_frame, timeline.play_pause, timeline.set_work_start, timeline.set_work_end, timeline.set_markers, exposure.set_span, exposure.write, exposure.toggle_key, sheet.write_action, property.set_base, keyframe.add_remove, keyframe.move, keyframe.set_interp, keyframe.set_path, keyframe.set_kind, keyframe.set_roving, property.separate, property.set_expression, effect.add, effect.delete, effect.toggle_bypass, effect.set_parameters, effect.move_up, effect.move_down, effect.move, viewer.fit, viewer.zoom_100, viewer.toggle_checkerboard, viewer.toggle_alpha, render.preview_current, export.sequence, app.command_palette | pass |
...
| `composition.set_settings` is reached by | a command the window answers | a command the window answers | pass |
| `composition.duplicate` is reached by | a command the window answers | a command the window answers | pass |
| `composition.delete` is reached by | a command the window answers | a command the window answers | pass |
...
| `layer.set_gain` is reached by | a command the window answers | a command the window answers | pass |
...
| `layer.add_solid` is reached by | a command the window answers | a command the window answers | pass |
| `solid.set` is reached by | a command the window answers | a command the window answers | pass |
| `mask.add` is reached by | a command the window answers | a command the window answers | pass |
| `mask.add_remove_key` is reached by | a command the window answers | a command the window answers | pass |
| `mask.set_path` is reached by | a command the window answers | a command the window answers | pass |
| `mask.set` is reached by | a command the window answers | a command the window answers | pass |
| `mask.delete` is reached by | a command the window answers | a command the window answers | pass |
| `layer.add_shape` is reached by | a command the window answers | a command the window answers | pass |
| `shape.add` is reached by | a command the window answers | a command the window answers | pass |
| `shape.add_remove_key` is reached by | a command the window answers | a command the window answers | pass |
| `shape.set_path` is reached by | a command the window answers | a command the window answers | pass |
| `shape.set` is reached by | a command the window answers | a command the window answers | pass |
| `shape.delete` is reached by | a command the window answers | a command the window answers | pass |
| `path.add_point` is reached by | a command the window answers | a command the window answers | pass |
| `path.remove_point` is reached by | a command the window answers | a command the window answers | pass |
| `layer.add_null` is reached by | a command the window answers | a command the window answers | pass |
| `property.link` is reached by | a command the window answers | a command the window answers | pass |
| `timesheet.import` is reached by | a command the window answers | a command the window answers | pass |
| `sheet.print` is reached by | the page, with no request | the page, with no request | pass |
...
| `exposure.write` is reached by | a command the window answers | a command the window answers | pass |
| `exposure.toggle_key` is reached by | a command the window answers | a command the window answers | pass |
| `sheet.write_action` is reached by | a command the window answers | a command the window answers | pass |
...
| `keyframe.set_kind` is reached by | a command the window answers | a command the window answers | pass |
| `keyframe.set_roving` is reached by | a command the window answers | a command the window answers | pass |
| `property.separate` is reached by | a command the window answers | a command the window answers | pass |
...
| document 24 gives `layer.add_solid` the shortcut | Ctrl+Y | Ctrl+Y | pass |
| and Ctrl+Y is bound | yes | yes | pass |
| document 24 gives `layer.add_null` the shortcut | Ctrl+Alt+Shift+Y | Ctrl+Alt+Shift+Y | pass |
| and Ctrl+Alt+Shift+Y is bound | yes | yes | pass |
...
| document 24 gives `sheet.print` the shortcut | Ctrl+P | Ctrl+P | pass |
| and Ctrl+P is bound | yes | yes | pass |
| document 24 gives `composition.set_settings` the shortcut | Ctrl+K | Ctrl+K | pass |
| and Ctrl+K is bound | yes | yes | pass |
| document 24 gives `layer.copy` the shortcut | Ctrl+C | Ctrl+C | pass |
| and Ctrl+C is bound | yes | yes | pass |
| document 24 gives `layer.paste` the shortcut | Ctrl+V | Ctrl+V | pass |
| and Ctrl+V is bound | yes | yes | pass |
| document 24 gives `layer.toggle_lock` the shortcut | Ctrl+L | Ctrl+L | pass |
| and Ctrl+L is bound | yes | yes | pass |
| document 24 gives `layer.toggle_visibility` the shortcut | Ctrl+Alt+V | Ctrl+Alt+V | pass |
| and Ctrl+Alt+V is bound | yes | yes | pass |
| document 24 gives `mask.add` the shortcut | G for the pen, Q for the rectangle and the ellipse | G for the pen, Q for the rectangle and the ellipse | pass |
...
| and Alt+G presses | $('checker') | $('checker') | pass |
