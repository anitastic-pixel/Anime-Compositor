...
2. **Adding it.** Choose it. A card named Selective Colour Blur appears with a **Blur, 0 to
   200** box at 12 and a **Colours** row that says no colour is chosen. The picture does not
   change yet.
3. **Choosing colours.** Click **Add a colour**. In the colour window, type `f6d6be` into its
   hex box and close it. A swatch with `#f6d6be` beside it appears. Still no change in the
   picture: skin has nothing to mix with. Add a second colour, `dba08e`. Now the edge between
   skin and shadow turns into a soft ramp, as in the proposal's first picture.
...
6. **Changing and removing a colour.** Click a swatch and pick a different colour, such as pure
   white: the edge it was softening goes back to sharp. Click the ✕ beside a swatch: it is
   removed and its edges go back to sharp.
7. **One step off does nothing.** Change the skin swatch to `f6d6bf`, one step off. The skin
   is no longer softened. Change it back to `f6d6be`.
...
- The colour window's eyedropper, if it has one, reads the screen, which may not be the drawing's
  exact colour. Typing the hex value is the sure way. Choosing a colour by clicking the picture
  is not in D-87.
