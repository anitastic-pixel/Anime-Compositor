...
`verification/B-08_window_shell.md` shows what that was in a window before the cache existed: the viewer played 66 frames and dropped 79 across about six seconds, roughly 92 ms of wall clock per frame delivered against the 41.7 ms a 24 fps shot allows.
...
3. **The export path neither reads nor writes the cel cache or the effect cache.** A full-resolution preview and an export of the same frame currently differ in 0 of 8,294,400 samples, and T-08's tables are byte-exact. A cache that can change one exported sample is the only failure mode here that would matter, and keeping it out of the export path costs less than proving it harmless inside one.

   *Amended on 2026-09-12, per D-48.* This bound was first written as "Export neither reads the cache nor writes it", with the Consequences section below saying "The export path gains nothing." Two units in a row - P-03(b) and P-10 - read that pair of sentences as possibly forbidding anything that makes export faster, including threads, and left the question open rather than guess. It does not. **A thread count is not a cache.** ADR-011 has rendered every frame tiled across the rayon pool since B-05a and `crate::export` renders through the same `compose::render_frame` as everything else, so threads were on the export path before there was a cache to keep off it; `verification/D-48_export_threads.md` checks on every build that the same eight export frames come out identical in a pool of one thread and in a pool of twenty-four. What this bound forbids is a remembered *result* reaching an exported sample. Nothing that leaves every exported byte unchanged is forbidden by it. The original wording is left in the record above rather than deleted, because what a bound was once read to mean is part of what it means.
...
The window-level half is no longer owed, and this record said it was until 2026-09-16. `verification/B-08_window_shell.md` counts playback in dropped frames by photographing a running window. When this was written that picture had not been retaken, because the capture script's synthetic keystroke stopped reaching the webview and three attempts produced a picture of an idle viewer rather than a playing one. It was retaken later on 2026-09-05, reading 59 frames played and 15 dropped, and again on 2026-09-08, reading 42 played and 31 dropped, against 66 and 79 before the cache. The number a person sees has therefore been re-measured, and the artifact attributes the difference between the two retakes to a slower machine-day rather than to the window.
