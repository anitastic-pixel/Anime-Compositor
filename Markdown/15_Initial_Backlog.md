...
B-08 / R-06a / Viewer, frame stepping, work-area playback, resolution selection, and the transport chosen by SP-05. Depends on B-05a. No cache. Playback holds real time and drops the frames it cannot deliver, reporting how many it skipped, per D-32; the preview starts at draft resolution with an always-visible indicator of which resolution is showing, per D-33. The headless half is done - B-08a, merged - and so is the headless middle: `src/preview.rs` carries the resolution choice, the plan scale and a playback clock driven by an elapsed time the caller supplies, with `verification/B-08_preview_table.md` as its artifact. What remains is the window, the transport, the wall-clock loop wired to that playback clock, the on-screen resolution indicator and the screenshots.
