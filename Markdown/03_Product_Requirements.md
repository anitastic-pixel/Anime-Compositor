...
R-03 / C / Layers: add, remove, reorder, rename, lock and hide raster layers; animate 2D position, anchor, scale, rotation and opacity with hold, linear and eased interpolation, position optionally along a curved path. T-03; B-05. The third mode was added on 2026-09-12 by D-52, at the owner's request after the third sitting with the window, and it is the only widening this requirement has had. An ease is a cubic Bezier of value against time, written as the two inner control points of a curve whose ends are pinned at (0,0) and (1,1) - the shape CSS writes as `cubic-bezier(x1, y1, x2, y2)` and the shape After Effects draws as two handles in its value graph. It belongs to the segment that starts at the keyframe carrying it, exactly as hold and linear do, so nothing about document 20's keyframe model changes shape. **What it did not add was a motion path**: an eased position moves along a straight line between its keys, faster and slower, and a curve *through* the keys in space was a separate decision D-52 left open. That decision is D-53, taken on 2026-09-12, and it is the second and last widening this requirement has had. A path is a cubic Bezier in composition pixels with a handle either side of each position keyframe, stored as offsets in pixels from the key, and it is parameterized by the fraction of the way through the segment rather than by distance along the curve - so an ease and a path compose without either one deciding the other. A position keyframe with no handles is the straight line it has always been, which is why nothing above this sentence is retracted by it.
...
R-15 / L / Handoff: evaluate EXR and WAV first, then other formats, based on actual need. Each format needs its own conformance fixtures. T-14; B-16. Contract for EXR: D-62, accepted 2026-09-17. WAV is deferred by the owner on 2026-09-17 (D-63).
